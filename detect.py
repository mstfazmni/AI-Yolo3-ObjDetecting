...
tfp.compat.v1.disable_eager_execution()
...
def main(type, iou_threshold, confidence_threshold, input_names):
    ...
    if type == 'images':
        ...
        inputs = tfp.compat.v1.placeholder(tfp.float32, [batch_size, *_MODEL_SIZE, 3])
        detections = model(inputs, training=False)
        saver = tfp.compat.v1.train.Saver(tfp.compat.v1.global_variables(scope='yolo_v3_model'))

        with tfp.compat.v1.Session() as sess:
        ...
        inputs = tfp.compat.v1.placeholder(tfp.float32, [1, *_MODEL_SIZE, 3])
        detections = model(inputs, training=False)
        saver = tfp.compat.v1.train.Saver(tfp.compat.v1.global_variables(scope='yolo_v3_model'))
